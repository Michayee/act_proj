...
def run_exe(simu_folder: str, executable_name: str, timeout: float = 40):
    '''
    Run .exe file
    '''
...
class ExpTarget:
        ...
        if os.path.exists(exe_folder):
            for exp_type in self.exp_config:
                if os.name == 'nt':
                    ...
                    shutil.copy(os.path.join(exe_folder, f'{exp_type}'),os.path.join(simu_folder, f'{exp_type}'))
        else:
    ...
    def conduct_simu(self,
        ...
        for exp_type, exp_specs in self.exp_config.items():
            if exp_type in ['drained_cyclic_HCT',
                            'drained_cyclic_HCT_5cyc',
                            'undrained_cyclic_HCT',
                            'undrained_cyclic_HCT_liqcyc',
                            'undrained_cyclic_HCT_5cyc'
        ...
        if if_clear_exe:
            clear_exe(simu_folder) # not work for linux
...
class EvalCycliq:
    ...
    def eval_obj_1(self, para):
        ...
        temp_name = f'temp_simu_{str(int(temp_time0*10))}_{str(temp_pid)}'
